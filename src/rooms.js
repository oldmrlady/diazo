...
export function renderRoomList() {
  ...
  state.rooms.forEach(r => {
    ...
    el.innerHTML = `<div class="room-dot" style="background:${COLORS[r.colorIdx].b}"></div><div class="room-name">${r.name}</div><button class="dup-room" title="Duplicate room">${dupSvg}</button><button class="del-room">✕</button>`;
    el.addEventListener('click', () => setActiveRoom(r.id));
    el.querySelector('.dup-room').addEventListener('click', e => duplicateRoom(r.id, e));
    el.querySelector('.del-room').addEventListener('click', e => deleteRoom(r.id, e));
    list.appendChild(el);
  });
